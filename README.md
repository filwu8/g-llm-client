...
- 新增 macOS 与 Windows 桌面悬浮伙伴，提供蓝色和金色外观、快捷对话入口、状态提示与可配置的陪伴提醒。
- GPT-5.6 系列模型支持默认、低、中、高推理强度，可在完整聊天和快速对话的模型菜单中直接选择。
- 模型正在回复时可随时停止生成；超时、限流、网关波动和临时网络异常提供更清晰的提示、立即重试与倒计时自动恢复。
- 联网搜索增强实体识别、新闻搜索、结果相关性排序和去重，减少偏离用户问题的无关结果。
- 扩充医疗健康、法律财税、建筑工程、内容创作、软件开发、教育生活等专业助手模板，并为高风险场景补充边界提示。
- “关于本系统”新增源码许可证、第三方声明、商业授权和商标政策入口，发布包同步携带完整法律文件与第三方组件清单。
